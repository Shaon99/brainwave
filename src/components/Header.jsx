    ...
    >
      <div className="flex items-center px-5 lg:px-7.5 xl:px-10 max-lg:py-4">
          ...
          <div className="relative z-2 flex flex-col items-center justify-center m-auto lg:flex-row">
            {navigation.map((item) => (
              <NavLink
                ...
                onClick={handleClick}
                className={({ isActive }) =>
                  `block relative font-code text-2xl uppercase transition-colors hover:text-color-1 ${item.onlyMobile ? "lg:hidden" : ""
        ...
        <NavLink
          to="/sign-up"
          className={({ isActive }) =>
            `button hidden mr-8 transition-colors hover:text-n-1 lg:block ${isActive ? "text-n-1" : "text-n-1/50"
            }`
          }
        >
          New account
